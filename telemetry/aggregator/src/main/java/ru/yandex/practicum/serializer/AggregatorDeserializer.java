...
public class AggregatorDeserializer implements Deserializer<SpecificRecordBase> {
    ...
    public SpecificRecordBase deserialize(String topic, byte[] bytes) {
        try {
            if (bytes != null) {
                ...
                switch (topic) {
                    case AggregatorTopics.TELEMETRY_SENSORS_V1:
                        reader = new SpecificDatumReader<>(SensorEventAvro.getClassSchema());
                        break;
                    case AggregatorTopics.TELEMETRY_SNAPSHOTS_V1:
